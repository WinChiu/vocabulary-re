...
export function libraryView(state, list) {
  ...
  return `<div class="page-heading compact library-heading"><h1>Library</h1><div class="heading-actions">${button('import', icon('upload-simple'), 'icon-button', 'aria-label="Import CSV" title="Import CSV"')}${button('add', icon('plus'), 'icon-button', 'aria-label="Add word" title="Add word"')}</div></div><section class="library-panel"><form id="library-filters"><div class="search-wrap">${icon('magnifying-glass')}<input name="search" aria-label="Search library" placeholder="Search words, meanings, or notes…" value="${e(f.search)}"><div class="list-meta"><span>${list.length} ${f.type === 'phrase' ? 'phrases' : 'words'}</span><span>NEWEST FIRST</span></div>${button('toggle-library-filters', icon('sliders-horizontal'), 'filter-toggle', `aria-label="Filters" aria-expanded="${state.libraryFiltersOpen}" aria-controls="library-filter-options"` )}</div><div id="library-filter-options" class="filter-row" ${state.libraryFiltersOpen ? '' : 'hidden'}>${select('status', 'Status', statusOptions, f.status)}${select(
    ...
    f.type,
  )}${select('category', 'Category', categoryOptions(state.cards), f.category)}<label class="check-label"><input name="starred" type="checkbox" ${f.starred ? 'checked' : ''}>${icon('star')} Starred only</label></div></form><div class="word-list">${
    list.length
      ? list
          .map(
            (c) =>
              `<article class="word-row" data-action="preview" data-id="${e(c.id)}"><button class="word-open" data-action="preview" data-id="${e(c.id)}"><span class="word-name">${e(c.word_en)}</span><span class="word-meaning">${e(c.meaning_zh)}</span></button><div class="word-metadata"><span class="word-category">${icon('tag')}${e(c.category || 'Uncategorized')}</span>${badge(c)}</div><div class="row-actions">${button('star', icon(c.is_starred ? 'star-fill' : 'star'), `icon-button ${c.is_starred ? 'starred' : ''}`, `data-id="${e(c.id)}" aria-label="${c.is_starred ? 'Unstar' : 'Star'} ${e(c.word_en)}" aria-pressed="${c.is_starred}"`)}${button('edit', icon('pencil-simple'), 'icon-button', `data-id="${e(c.id)}" aria-label="Edit ${e(c.word_en)}"`)}${button('delete', icon('trash'), 'icon-button danger', `data-id="${e(c.id)}" aria-label="Delete ${e(c.word_en)}"`)}</div></article>`,
          )
          .join('')
